...
from custom_pd_embedding.model import MLP
...
# tensorBoard
tensorBoard_path = "runs_manmade_mlp"
...
def train(args, model, trainLoader, testLoader):
    ...
    # save model dict
    model_save_path = PROJECT_ROOT / "custom_pd_embedding/model/mlp"
    torch.save(model.state_dict(), model_save_path / f"mlp_{time_now}.pth")
...
if __name__ == "__main__":
    ...
    # model
    model = MLP(input_size=MLP_INPUT_DIM, output_size=len(np.unique(label_array))).to(
        device
    )
